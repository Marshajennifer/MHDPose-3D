...
def aggregate_hypotheses_torch(predicted, mode='mean', trim_ratio=0.2, hyp_dim=2):
    """
    Aggregate multiple pose hypotheses along hyp_dim.
    Three different supported modes have used: mean, median, trimmed_mean
    """
    if mode == 'mean':
        return torch.mean(predicted, dim=hyp_dim, keepdim=False)
    if mode == 'median':
        return torch.median(predicted, dim=hyp_dim, keepdim=False).values
    if mode == 'trimmed_mean':
        h = predicted.shape[hyp_dim]
        k = int(h * trim_ratio)
        if k <= 0 or 2 * k >= h:
            return torch.mean(predicted, dim=hyp_dim, keepdim=False)
        sorted_vals, _ = torch.sort(predicted, dim=hyp_dim)
        slicer = [slice(None)] * sorted_vals.dim()
        slicer[hyp_dim] = slice(k, h - k)
        trimmed = sorted_vals[tuple(slicer)]
        return torch.mean(trimmed, dim=hyp_dim, keepdim=False)
...
#jbest; mean_pos=True P-aggr
def mpjpe_diffusion_all_min(predicted, target, mean_pos=False, agg_mode='mean', trim_ratio=0.2):
    """
    Mean per-joint position error (i.e. mean Euclidean distance),
    often referred to as "Protocol #1" in many papers.
    """
    ...
    if not mean_pos:
        ...
        t = predicted.shape[1]
        mean_pose = aggregate_hypotheses_torch(predicted, mode=agg_mode, trim_ratio=trim_ratio, hyp_dim=2)
...
#p-best and p-aggr
def mpjpe_diffusion(predicted, target, trim=0.20, mean_pos=False, agg_mode='mean', trim_ratio=None):
    """
    Mean per-joint position error (i.e. mean Euclidean distance),
    often referred to as "Protocol #1" in many papers.
    """
    #assert predicted.shape == target.shape
            #p-best, not mean_pose=True because mean_pos=False
    if not mean_pos:
        t = predicted.shape[1]
        h = predicted.shape[2]
        # print(predicted.shape)
...
def mpjpe_diffusion_3dhp(predicted, target, valid_frame, mean_pos=False, agg_mode='mean', trim_ratio=0.2):
    """
    Mean per-joint position error (i.e. mean Euclidean distance),
    often referred to as "Protocol #1" in many papers.
    """
    ...
    if not mean_pos:
        ...
        t = predicted_valid.shape[1]
        mean_pose = aggregate_hypotheses_torch(predicted_valid, mode=agg_mode, trim_ratio=trim_ratio, hyp_dim=2)
...
def p_mpjpe_diffusion_all_min(predicted, target, mean_pos=False, agg_mode='mean', trim_ratio=0.2):
    """
    Pose error: MPJPE after rigid alignment (scale, rotation, and translation),
    often referred to as "Protocol #2" in many papers.
    """
    ...
    if not mean_pos:
        ...
        predicted = aggregate_hypotheses_torch(predicted, mode=agg_mode, trim_ratio=trim_ratio, hyp_dim=2)
...
def p_mpjpe_diffusion(predicted, target, mean_pos=False, agg_mode='mean', trim_ratio=0.2):
    """
    Pose error: MPJPE after rigid alignment (scale, rotation, and translation),
    often referred to as "Protocol #2" in many papers.
    """
