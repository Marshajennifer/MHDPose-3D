...
def draw_3d_image(pred_all, gt_all, skeleton, azim, sub, act, cam, frame_to_show=None):
    #frame = 200
    # for frame in range(gt_all.shape[0]):
    #     if frame %5 != 0:
    #         continue
    if frame_to_show is None:
        frames = range(gt_all.shape[0])
    else:
        frames = [frame_to_show]

    for frame in frames:
        if frame_to_show is None and frame %5 != 0:
            continue
        ...
        #for t in range(pred.shape[0]-1, pred.shape[0]):
            ...
            ax.set_zticklabels([])
            ax.dist = 8
            fig.suptitle("%s_%s_%d_frame%d_t%d_azim%d" % (sub, act, cam, frame, t, azim_delta))
            #ax.set_title("timestep %d" % t)  # , pad=35
            # ax.set_title("Cross dataset")  # , pad=35
            ax.get_xaxis().set_visible(False)
            ax.get_yaxis().set_visible(False)
            ...
            if frame_to_show is not None:
                ...
                plt.close()
# #To draw all hypothesis predictions and r,g,b at all timesteps r->pred, g->aggr, b->groundtruth
def draw_3d_image_select(pred_all, gt_all, skeleton, azim, sub, act, cam, gt_2d, pred_2d, frame_to_show=None):
    ...
    for frame in frames:
        ...
        #for t in range(pred.shape[0]-1, pred.shape[0]):
            ...
            for j, j_parent in enumerate(parents):
                ...
                # col = '#ff69b4' #pink, aggregated hypotheses
                col = '#c1121f' #red, aggregated hypotheses
...
# To draw all red predictions at all timesteps uncomment this and comment the one below and above(comment other draw_3d_image_select function if we are using it)
...
#         pred_s = (pred_s - pred_s[:, :, 0:1]) * 1000
#         gt = (gt - gt[0:1]) * 1000
# # for t in range(pred_all.shape[0]): to print all ODEtimestep outputs, including noisy t0 to tn=sampling/integration steps.
#         for t in range(pred_all.shape[0]):
#             fig = plt.figure()
#             ax = fig.add_subplot(111, projection='3d')
...
#                 # Plot best prediction (red)
#                 col = '#c1121f'  # red
#                 ax.plot([pred_s[t, 0, j, 0], pred_s[t, 0, j_parent, 0]],
#                         [pred_s[t, 0, j, 1], pred_s[t, 0, j_parent, 1]],
#                         [pred_s[t, 0, j, 2], pred_s[t, 0, j_parent, 2]], zdir='z', c=col, linewidth=0.7)

#             plt.savefig("./plot/h36m/%s_%s_%d_frame%d_t%d_azim%d_best_only.png" % (sub, act, cam, frame, t, azim_delta), 
#                         bbox_inches="tight", pad_inches=0.0, dpi=300)
#             plt.close()



# # To draw only red, the best one, at last timestep(comment other draw_3d_image_select function if we are using it):
# def draw_3d_image_select(pred_all, gt_all, skeleton, azim, sub, act, cam, gt_2d, pred_2d):
#     import torch
#     gt_2d = torch.from_numpy(gt_2d)
#     t_sz, h_sz = pred_all.shape[0], pred_all.shape[1]
...
#         gt = (gt - gt[0:1]) * 1000
# # for t in range(pred_all.shape[0]): to print all ODEtimestep outputs, including noisy t0 to tn=sampling/integration steps.
#         for t in range(pred_all.shape[0] - 1, pred_all.shape[0]):
#             fig = plt.figure()
#             ax = fig.add_subplot(111, projection='3d')
...
## To draw red and green, best and mean(comment other draw_3d_image_select function if we are using it):
# def draw_3d_image_select(pred_all, gt_all, skeleton, azim, sub, act, cam, gt_2d, pred_2d):
#     import torch
#     gt_2d = torch.from_numpy(gt_2d)
#     t_sz, h_sz = pred_all.shape[0], pred_all.shape[1]
...
#         #for t in range(pred_all.shape[0]):  # Plot all timesteps
#         for t in range(pred_all.shape[0] - 1, pred_all.shape[0]):
#             fig = plt.figure()
#             ax = fig.add_subplot(111, projection='3d')
...
#                 # Plot mean prediction (green)
#                 col = '#1b4332'  # green
#                 ax.plot([pred_m[t, 0, j, 0], pred_m[t, 0, j_parent, 0]],
#                         [pred_m[t, 0, j, 1], pred_m[t, 0, j_parent, 1]],
#                         [pred_m[t, 0, j, 2], pred_m[t, 0, j_parent, 2]], zdir='z', c=col, linewidth=0.7)

#             plt.savefig("./plot/h36m/%s_%s_%d_frame%d_t%d_azim%d_best_and_mean.png" % (sub, act, cam, frame, t, azim_delta), 
#                         bbox_inches="tight", pad_inches=0.0, dpi=300)
#             plt.close()



## To draw red, green and blue predictions at last timestep only (comment other draw_3d_image_select function if we are using it).
# def draw_3d_image_select(pred_all, gt_all, skeleton, azim, sub, act, cam, gt_2d, pred_2d):
#     import torch
#     gt_2d = torch.from_numpy(gt_2d)
#     t_sz, h_sz = pred_all.shape[0], pred_all.shape[1]
...
#         # Plot only the last timestep
#         t = pred_all.shape[0] - 1
#         fig = plt.figure()
#         ax = fig.add_subplot(111, projection='3d')
...
#         plt.savefig("./plot/h36m/%s_%s_%d_frame%d_t%d_azim%d_rgb_last.png" % (sub, act, cam, frame, t, azim_delta), 
#                     bbox_inches="tight", pad_inches=0.0, dpi=300)
#         plt.close()
